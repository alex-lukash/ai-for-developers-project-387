...
**Deploy image.** The root [`Dockerfile`](Dockerfile) builds a single full-stack image via a multi-stage build (compile spec → build SPA → build boot jar): a Spring Boot container serving **both** the React SPA and the `/api` endpoints on one port. It listens on the `PORT` env var (default 8080) — `backend/src/main/resources/application.yaml` binds `server.port: ${PORT:8080}`, the SPA static files are bundled into the backend's `resources/static` at image-build time, and `WebConfig.kt` adds SPA-fallback routing so client-side routes resolve to `index.html`. This is what the Hexlet check builds and runs (`PORT` is supplied by the check and at deploy).
...
**Commit conventions.** All commits **and PR titles** must follow [Conventional Commits](https://www.conventionalcommits.org/) — see [`CONTRIBUTING.md`](CONTRIBUTING.md) for the types, allowed scopes (`frontend`/`backend`/`spec`/`e2e`/`ci`/`deps`/`release`/`repo`), and breaking-change rules. This is enforced by commitlint in CI ([`.commitlintrc.json`](.commitlintrc.json)) and consumed by **release-please** ([`.github/workflows/release-please.yml`](.github/workflows/release-please.yml) + `release-please-config.json`) to version and changelog the packages automatically. Write commits accordingly (the `Co-Authored-By:` trailer is a valid footer); never hand-bump versions or changelogs.

**Always bump the version.** Every change must land as a release-bumping commit so the version always advances: use `feat`/`fix`/`perf` (or a breaking `!`) for substantive work — `chore`/`docs`/`ci`/`build`/`refactor`/`test`/`style` bump nothing. Every path maps to a package: `frontend`/`backend`/`spec` for their dirs, and the **`repo`** package (path `"."`) for root/infra files (`Dockerfile`, `.github/**`, root configs), so even root-only changes cut a release. **Never** add a `release-as` pin to `release-please-config.json` — it freezes versioning (it is what previously stuck the repo at `0.1.0`).
